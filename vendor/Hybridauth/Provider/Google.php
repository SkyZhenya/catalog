...
class Google extends OAuth2Template
{
	...
	*/
	...
	{
		$response = $this->signedRequest( "https://www.googleapis.com/plus/v1/people/me" );
		...
		$profile->setFirstName  ( $parser( 'name' )->givenName );
		$profile->setLastName   ( $parser( 'name' )->familyName );
		$profile->setDisplayName( $parser( 'displayName' ) );
		$profile->setPhotoURL   ( $parser( 'picture'     ) );
		$profile->setProfileURL ( $parser( 'link'        ) );
		$profile->setGender     ( $parser( 'gender'      ) );
		$profile->setEmail      ( $parser( 'emails'      )[0]->value );
		$profile->setLanguage   ( $parser( 'locale'      ) );
		$photoURL = $parser( 'image'      )->url;
		$pos = strpos($photoURL,'?sz=');
		if ($pos !== false) {
			$photoURL = substr($photoURL, 0, $pos);
		}
